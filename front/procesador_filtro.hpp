...
payload["campos_bus"].get<std::vector<std::string>>(); std::vector<std::string>
valores = payload["valores_bus"].get<std::vector<std::string>>(); std::string
operador = payload["operador"].get<std::string>(); std::vector<std::string>
columnas = payload["columnas"].get<std::vector<std::string>>();
    ...
    std::vector<std::vector<std::string>> resultado = dbmanager->buscar(campos,
valores, operador, columnas, atributo_orden, direccion_orden);

    return resultado; // json automáticamente lo convierte si es
